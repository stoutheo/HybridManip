#-----------------------------------------------------------------------------#
# This file is an implementation of the Cnt2Cnt - Sw2Cnt bimanual primitive
#   ......
#-----------------------------------------------------------------------------#
...
optProb.addTorqueCntConst(Bee_name)

# add friction cone constraint
optProb.addFrictionConeCntConst(Bee_name)

# add distance from the shape constraint
optProb.addSwingContConst(Bee_name, withCnt=True)

# add integration constraint for the contact-swing end effector
optProb.addSwingLocalIntegConst(Bee_name)
...
# basic nlp solver
nlp = {'x':optProb.varVect, 'p':ca.vertcat(Acnt_loc, goal, K, D, cstW), 'f':f, 'g':optProb.g}

S = ca.nlpsol('S_1Cnt_1SwCnt', 'ipopt', nlp, {'ipopt.print_level':5, 'ipopt.hessian_approximation':"exact"})
...
bPosSw_f, bVelSw_f = optProb.buildSwingBoundFun(Bee_name, SwCnt = True)
...
# position bounds
max_p = 1
Xlbxf, Xubxf = bPos_f([curr, curr, currTheta], [-max_p, -max_p, -max_p], [max_p, max_p, max_p])
...
BpolarPoslbx, BpolarPosubx = bPosSw_f(-ca.pi/2, 0.3, -ca.pi, 0.1, -ca.pi/2, .5, -ca.pi/2)
...
lbx =  np.array(Xlbxf)[0,:].tolist() + np.array(dXlbxf)[0,:].tolist() + dtlbx + \
       ...
       np.array(BpolarPoslbx)[0,:].tolist() + np.array(BpolarVellbx)[0,:].tolist()  + np.array(BfCntlbx)[0,:].tolist() + np.array(BfcoefCntlbx)[0,:].tolist()

ubx =  np.array(Xubxf)[0,:].tolist() + np.array(dXubxf)[0,:].tolist() + dtubx + \
       np.array(AfCntubx)[0,:].tolist() + np.array(AfcoefCntubx)[0,:].tolist() + \
       np.array(BpolarPosubx)[0,:].tolist() + np.array(BpolarVelubx)[0,:].tolist()  + np.array(BfCntubx)[0,:].tolist() + np.array(BfcoefCntubx)[0,:].tolist()
...
p = [ca.pi/8, goalX, goalY, goalPhi, 0, 5, 5, 25, 25, 25, 10, 10, 0] #[ca.pi + ca.pi/4]
...
getPosF, getVelF, getTimeF, getForcesAF, getForcesBF, getCartAF, getPolarOrientAF, getCartBF, getPolarOrientBF \
                                                = optProb.decodeSolution(Aee_name, Bee_name, SwCnt=True, CntSw=False)
...
Bforces3D = np.hstack( (np.tile(0,[3, int(pos.size1()/3) - int(Bforces.size1()/3)]), Bforces3D))
...
Bcart2D = np.array(getCartBF(x_opt, 0))
