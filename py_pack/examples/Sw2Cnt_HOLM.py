#-----------------------------------------------------------------------------#
# This file is an implementation of the Sw2Cnt primitive
#   ......
#-----------------------------------------------------------------------------#
...
optProb.addSwingContactEndEffVars(ee_name)
...
optProb.addTorqueCntConst(ee_name)

# add friction cone constraint
optProb.addFrictionConeCntConst(ee_name)

# add distance from the shape constraint
optProb.addSwingContConst(ee_name, withCnt=True)

# add integration constraint for the contact-swing end effector
optProb.addSwingLocalIntegConst(ee_name)
...
# basic nlp solver
nlp = {'x':optProb.varVect, 'p':ca.vertcat(goal, K, D), 'f':f, 'g':optProb.g}
...
# start the hand below the object
polarPoslbx = [3*ca.pi/2, 0.5] + [-ca.inf]*polarPosSw.size1()*(polarPosSw.size2()-1)
polarPosubx = [3*ca.pi/2, 0.5] + [ca.inf]*polarPosSw.size1()*(polarPosSw.size2()-1)
...
lbx =  Xlbx + dXlbx + dtlbx + polarPoslbx + polarVellbx + fCntlbx + fcoefCntlbx
ubx =  Xubx + dXubx + dtubx + polarPosubx + polarVelubx + fCntubx + fcoefCntubx
...
p = [goalX, goalY, goalPhi,  0, 0, 0, 0, 0, 0]
...
forces3D = np.hstack( (np.tile(0,[3, int(pos.size1()/3) - int(forces.size1()/3)]), forces3D))
...
for i in range(num_samples):
    ...
    if i < optProb.CtrIntValpPhase[0]:
        ee_pos.append(np.array(ca.mtimes(operators.R(pos3D[2,i]), operators.circlePt(polar2D[1,i], polar2D[0,i]))))
    else:   # if in contact with the object
        ee_pos.append(np.array(ca.mtimes(operators.R(pos3D[2,i]), operators.circlePt(polar2D[1,-1], polar2D[0,-1]) )))
